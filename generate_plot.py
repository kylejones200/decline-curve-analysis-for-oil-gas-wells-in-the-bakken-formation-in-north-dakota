...
def hyperbolic_cumulative(t, qi, Di, b):
    return (
        (qi**b) / ((1 - b) * Di) * (qi ** (1 - b) - (qi / (1 + b * Di * t)) ** (1 - b))
    )
...
fig, axes = plt.subplots(
    2, 2, figsize=tuple(config.get("output", {}).get("figsize", [14, 10]))
)
...
ax1.axhline(
    10, color="gray", linestyle="--", linewidth=1, label="Economic Limit (10 bbl/d)"
)
...
ax2.plot(
    forecast_months,
    forecast_cumulative / 1000,
    "k--",
    linewidth=1.5,
    alpha=0.7,
    label="Forecast (240 mo)",
)
...
for idx, (b_val, label_base) in enumerate(
    [
        (0, "Exponential"),
        (0.3, "Low Hyperbolic"),
        (0.5, "Mid Hyperbolic"),
